  ...
  if(isset($_GET['search']) && isset($_GET['page']) && isset($_GET['radio'])){
        if($_GET['search'] != ""){
          if($_GET['isCorrected'] == '1') {
            switch($_GET['radio']){
                case 0:
                    $path = "new_searcher.py";
                    break;
                case 1:
                    $path = "new_pdf_searcher.py";
                    break;
            }
          } else {
            switch($_GET['radio']){
                case 0:
                    $path = "wrong_searcher.py";
                    break;
                case 1:
                    $path = "wrong_pdf_searcher.py";
                    break;
            }
          }
          // shell_exec("chmod 777 *");
           // $command = escapeshellcmd('python '.$path.' '.str_replace(' ','{}',$_GET['search']).' > files.txt');
           $output = shell_exec(escapeshellcmd('python '.$path.' '.str_replace(' ','{}',$_GET['search']).' 2>&1'));
           // var_dump($output);
         ...
         // var_dump($start);
            ...
            for ($i=$start;$i<=$stop;$i++) {
              ...
              for($i=1;$i<=$stop;$i++){
                  ...
                  echo '<li '.$active.'onclick="searchq('.$i.',1)"><a href="#">'.$i.'</a></li>';
              }
              echo '</ul>';
      }else{
